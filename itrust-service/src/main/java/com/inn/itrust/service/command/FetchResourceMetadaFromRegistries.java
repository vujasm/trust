 ...
 *
 */
public class FetchResourceMetadaFromRegistries {
	
	private final SparqlGraphStoreManager graphStoreManager;
	private final List<SparqlGraphStoreManager>   externalGraphStoreMgrs;
	
	
	public FetchResourceMetadaFromRegistries(final SparqlGraphStoreManager graphStoreManager, final List<SparqlGraphStoreManager> externalGraphStoreMgrs) {
	...
	public OntModel apply(URI uri, boolean fetchFromExternalRegistries, boolean useMappedLocations, boolean fetchFromInternalRegirsty) {
		...
		if (externalModel == null && useMappedLocations) {
			...
			externalModel = new ModelFether().fetch(uri, "TURTLE", graphStoreManager.getOntModelSpec());
		}
		...
		OntModel model = ModelFactory.createOntologyModel(graphStoreManager.getOntModelSpec(), modelUnion);

		return model;
	}
	
	/**
	 * goes thru a list of spqrqlEndpoints and tries to find any metadata for
	 * the service
	 * 
	 * @param uri
	 *            - service URI.
